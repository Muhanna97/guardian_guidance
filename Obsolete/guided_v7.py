...
def guardianInit():
    vehicle.airspeed = 10;
    gotoGuardian3.reachedMinorLocation = 1 # start up with 1
...
# def gotoGuardian2(targetLocation,gotoFunction = goto_position):
#     currentLocation = vehicle.location.global_relative_frame
#     targetDistance = get_distance_metres(currentLocation, targetLocation)
#
...
#
#         if wpDistance < 1.0: # handles small oscillations near a point
#             smallDistance = 1/wpDistance
#
#         if wpDistance <= targetDistance*0.01 or wpDistance < 0.3 or smallDistance > 2: #Just below target, in case of undershoot.
#             print "Reached target"
#             return 1
#     else:
...
class gotoGuardian3(mthread.MicroThread):
    def __init__(self, number):
        ...
        self.reachedMajorLocation = 0
        self.reachedMinorLocation = 1 # start up with 1
        self.wpNum = 0 # current wp we're flying to
        self.intermediateLocation = LocationGlobalRelative(50.130629, -12.793948, 100) # point between target wp and current location
        self.currentGoToLocation = LocationGlobalRelative(49.130629, -122.793948, 100) # location we're currently going to
        self.targetLocation = LocationGlobalRelative(49.130629, -122.793948, 100) # overall wp we want to go to
        self.lastRunTime =  dt.datetime.now()

    def createwp(self):
        self.targetLocation = LocationGlobalRelative(lats[self.wpNum],longs[self.wpNum],alts[self.wpNum])
    ...
    def step(self):
        ...
        if (stepTimer >= 0.1):
            ...
            if(self.checkIfPointReached(self.intermediateLocation)):
                self.reachedMinorLocation = 1

            if(self.checkIfPointReached(self.targetLocation)):
                self.reachedMajorLocation = 1
                self.reachedMinorLocation = 1
                self.wpNum += 1

            self.lastRunTime = dt.datetime.now()
            print "Wpnum, ", self.wpNum
    ...
    def getBearingGood(self, currentLocation, targetLocation):
    ...
    def genLocationOut(self,currentLocation, targetLocation, distanceOut):
        #http://www.movable-type.co.uk/scripts/latlong.html#destPoint
        bearing = self.getBearingGood(currentLocation, targetLocation)
        print "degrees", bearing*180/3.14159
        print currentLocation
        print targetLocation
        R = 6378137.0

        latitude = asin(sin(rad(currentLocation.lat))*cos(distanceOut/R) + cos(rad(currentLocation.lat))*sin(distanceOut/R)*cos(bearing))
        longitude = rad(currentLocation.lon) + atan2(sin(bearing)*sin(distanceOut/R)*cos(rad(currentLocation.lat)),cos(distanceOut/R)-sin(rad(currentLocation.lat))*sin(latitude))

        return LocationGlobalRelative(deg(latitude), deg(longitude), currentLocation.alt)

    def go(self):
        # gotoFunction = vehicle.simple_goto
        targetLoc = self.targetLocation
        currentLocation = vehicle.location.global_relative_frame
        targetDistance = get_distance_metres(currentLocation, targetLoc)

        print "Target distance: ", targetDistance
        if (targetDistance > 20 and self.reachedMinorLocation == 1): # if we're far from main target and have reached intermediate location, send another one
            self.intermediateLocation = self.genLocationOut(currentLocation, targetLoc, 20)
            print self.intermediateLocation
            self.reachedMinorLocation = 0
            self.currentGoToLocation = self.intermediateLocation # let's only send currentGoToLocation so we always know where it's going
            print "Going to intermediate location"
            vehicle.simple_goto(self.currentGoToLocation)
        elif(targetDistance <= 20): # if close to target, go right there
            self.currentGoToLocation = self.targetLocation
...
class TestMicroThread(mthread.MicroThread):
    def __init__(self, number, inputfunction):
        self.num = number
        self.step = inputfunction # essentially a function pointer to attach a function to the step method (which the scheduler runs)

class testFly(mthread.MicroThread):
    def __init__(self, number):
        self.num = number
        self.wpNum = 0
        self.createwp()
        self.lastRunTime =  dt.datetime.now()
        self.alternateWP = LocationGlobalRelative(49.130629, -122.793948, 100)

    def createwp(self):
        self.wp = LocationGlobalRelative(lats[self.wpNum],longs[self.wpNum],alts[self.wpNum])

    def step(self):
        stepTimer = secondsSince(self.lastRunTime)
        # run 10x per second
        if (stepTimer >= 0.1):
            # if (checkObstacle.obstacleFound):
            #     print "ffffffffffffffffffffffffffffffffffffffffffffffff"
            #     gotoGuardian2.hasSent = 0
            #     vehicle.simple_goto(self.alternateWP)# goto alternate point
            # else:
            if gotoGuardian2(self.wp) == 1: # go to waypoint
                self.wpNum += 1 # if we have reached the waypoint
                self.createwp()
                gotoGuardian2.hasSent = 0
            self.lastRunTime = dt.datetime.now()
...
class checkObstacle(mthread.MicroThread):
    obstacleFound = 0
    alternateWP = LocationGlobalRelative(49.130629, -122.793948, 100)

    def __init__(self, number):
        self.obstacle = LocationGlobalRelative(49.129487,-122.792134,100) # probably put curernt altitude here
        self.lastRunTime =  dt.datetime.now()
        self.obstacleFound = 0

    def step(self):
        timeSince = secondsSince(self.lastRunTime)
        if (timeSince >= 0.1):
            currentLocation = vehicle.location.global_relative_frame
            obstacleDistance = get_distance_metres(currentLocation, self.obstacle)
            if (obstacleDistance < 50):
                print "Obstacle is ", obstacleDistance, "metres away"
                self.obstacleFound = 1
                vehicle.simple_goto(self.alternateWP)# goto alternate point
...
mt1 = TestMicroThread(1, printSomething)
mt2 = gotoGuardian3(2)
# mt3 = checkObstacle(2)
