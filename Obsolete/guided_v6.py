...
def gotoVelocity(targetLocation):
    bearing = get_bearing(vehicle.location.global_relative_frame,targetLocation)
    ...
    condition_yaw(bearing) #point towards travel direction

    targetDistance = get_distance_metres(targetLocation,vehicle.location.global_relative_frame)

    if(targetDistance < 15):
        distanceScale = (targetDistance + 2)/5;
    if(targetDistance < 4):
        distanceScale = 0
    if (targetDistance < 2): # we have reached target
        distanceScale = 0
        return 1
    else:
        distanceScale = 1
...
class testFly2(mthread.MicroThread):
    def __init__(self, number):
        self.num = number
        self.wpNum = 0
        self.createwp()
    ...
    def step(self):
        ...
        if (stepTimer >= 2):
            ...
            #     vehicle.simple_goto(self.alternateWP)# goto alternate point
            # else:
            if gotoVelocity(self.wp) == 1: # go to waypoint
                self.wpNum += 1 # if we have reached the waypoint
                self.createwp()
            self.lastRunTime = dt.datetime.now()
