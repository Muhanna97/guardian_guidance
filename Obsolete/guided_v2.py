...
def gotoGuardian(targetLocation, gotoFunction=vehicle.simple_goto):
    """
    Moves the vehicle to a position dNorth metres North and dEast metres East of the current position.
...
#    targetLocation = get_location_metres(currentLocation, dNorth, dEast)
    ...
    while vehicle.mode.name=="GUIDED": #Stop action if we are no longer in guided mode.
        ...
        remainingDistance=get_distance_metres(vehicle.location.global_relative_frame, targetLocation)
        print "Distance to target: ", remainingDistance
        if remainingDistance < 1.0: # handles small oscillations near a point
            smallDistance = smallDistance + 1

        if remainingDistance<=targetDistance*0.01 or remainingDistance < 0.3 or smallDistance > 3: #Just below target, in case of undershoot.
            print "Reached target"
            break;
        time.sleep(2)
...
for i in range(len(lats)):
    print("Going towards point",i)
    point = LocationGlobalRelative(lats[i],longs[i],alts[i])
